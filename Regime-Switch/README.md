...
## MSGARCH Model (Markov-Switching GARCH)

This module implements a Markov-Switching GARCH model to detect volatility regimes in Bitcoin/USDT.

### Overview

This project uses the results found by [Ardia, D., Bluteau, K., & Rüede, M. (2019). Regime changes in Bitcoin GARCH volatility dynamics](https://doi.org/10.1016/j.frl.2018.08.009) to accurately model regime shifts in the volatility of Bitcoin (_BTC/USDT_).

### Features

- **GJR-GARCH(1,1) Model**: Fits a GJR-GARCH model with Skewed Student-T distribution to forecast volatility
- **Markov-Switching**: Applies 2-regime and 3-regime Markov-Switching models to GARCH forecasted volatilities
- **Regime Detection**: Identifies Low-Vol, Medium-Vol, and High-Vol regimes
- **Visualization**: Generates filtered and smoothed probability plots

### Files

- `msgarch_btcusd.py`: Main Python script with all functions and classes
- `MSGARCH_BTCUSD.ipynb`: Original Jupyter notebook
...
```python
from msgarch_btcusd import run_2_regime_model, run_3_regime_model
...
- `dates`: Date string in format 'YYYY-MM-DD HH:MM'

Or alternatively:
- `open_time`: Timestamp column (will be converted to dates)
