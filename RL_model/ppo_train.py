...
class TradingEnv:
    ...
    def reset(self)-> np.ndarray:
        ...
        # latest start must allow at least one step using t+1
        hard_max_start = self.T - 2

        if self.cfg.random_start and self.cfg.max_episode_steps is not None:
            max_start = min(hard_max_start, self.T - self.cfg.max_episode_steps - 1)
            max_start = max(max_start, self.cfg.start_index)
            self.episode_start = int(self.rng.integers(self.cfg.start_index, max_start + 1))
        else:
            self.episode_start = min(self.cfg.start_index, hard_max_start)
    ...
    def step(self, action: int):
        ...
        # t: current index, obs = X[t]
        new_pos = action

        # need t+1 exist
        terminated = (self.t >= self.T - 2)  # because we use close[t+1]
        truncated = False

        # next return (t -> t+1) - use log return for proper compounding
        ret_next = float(np.log((self.close[self.t + 1] + 1e-12) / (self.close[self.t] + 1e-12)))

        turnover = abs(new_pos - self.pos)
        cost_fee = self.fee * turnover
        cost_hold = self.hold_cost * abs(new_pos)  # hold cost on new position (common)

        # Real P&L (profit & loss) in log space - used for equity calculation
        # This represents the actual trading profit/loss
        pnl_log = new_pos * ret_next - cost_fee - cost_hold
        
        # Reward can be customized for training (e.g., add risk penalty, regularization, etc.)
        # By default, use the same as P&L, but you can modify this
        simple_ret = (self.close[self.t + 1] / (self.close[self.t] + 1e-12)) - 1.0
        reward = new_pos * simple_ret - cost_fee - cost_hold  # You can change this to any reward design
        ...
        # apply time-limit truncation
        if self.cfg.max_episode_steps is not None and self.steps >= self.cfg.max_episode_steps:
            truncated = True

        done = terminated or truncated

        # next obs is X[t] after increment, if done we can still return last valid obs
        obs = self.X[self.t] if not done else self.X[min(self.t, self.T - 1)]

        info = {
            "t": self.t,                 # now points to next state's index
            "ret_next": ret_next,
            "pos": self.pos,
            "turnover": turnover,
            "fee": cost_fee,
            "hold_cost": cost_hold,
            "pnl_log": pnl_log,          # real P&L for equity calculation
...
class RollingBuffer:
    ...
    def __init__(self,size,obs_dim,device):
        ...
        self.dones = torch.zeros((size,),device=device)  # kept for compatibility
        self.terminateds = torch.zeros((size,),device=device)  # true termination (not truncation)
    ...
    def add(self,obs,act_idx,reward,done,logprob,value,terminated=None):
        i = self.ptr
        self.obs[i] = obs
        self.actions[i] = act_idx
        self.rewards[i] = reward
        self.dones[i] = done  # kept for compatibility
        # terminated: True if truly terminated (not truncated), False otherwise
        # if not provided, use done (backward compatibility)
        if terminated is None:
            terminated = done
        self.terminateds[i] = float(terminated)
    ...
    def compute_gae(self,last_value,gamma,lam):
        ...
        for t in reversed(range(self.size)):
            # mask = 0 for terminated (no bootstrap), 1 for truncated/continuing (allow bootstrap)
            mask = 1.0 - self.terminateds[t]
    ...
    def train(self,total_updates = 200,log_every = 10):
        ...
        for upd in range(total_updates):
            ...
            for _ in range(self.cfg.rollout_steps):
                ...
                # extract terminated flag (true termination, not truncation)
                terminated = info.get("terminated", done and not info.get("truncated", False))
            ...
            for _ in range(self.cfg.update_epochs):
                for mb_obs, mb_act, mb_oldlog, mb_adv, mb_ret in buf.minibatches(self.cfg.minibatch_size):
                    # normalize advantages (PPO standard practice)
...
if __name__ == "__main__":
    ...
    # 默认使用训练数据，如果提供了参数则使用该参数
    TRAIN_CSV = sys.argv[1] if len(sys.argv) > 1 else "train_data.csv"
    POLICY_PATH = "ppo_policy.pt"
    
    cfg = PPOConfig()
    print(f"正在读取训练数据: {TRAIN_CSV}")
    bar = pd.read_csv(TRAIN_CSV)
